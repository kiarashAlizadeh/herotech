...
// CancelBid godoc
// @Summary      Retract a non-leading bid position
// @Description  Cancels bid stakes and unlocks reserved allocations instantly if bidder isn't holding the pole position
// @Tags         auctions
// @Produce      json
// @Param        X-Guild-ID  header    string  true  "Bidder Guild UUID"
// @Param        id          path      string  true  "Auction UUID"
// @Success      200  {object}  map[string]string
// @Failure      400  {object}  map[string]string
// @Router       /auctions/{id}/bid [delete]
func (h *AuctionHandler) CancelBid(c *gin.Context) {
	bidderStr := c.GetHeader("X-Guild-ID")
	bidderID, err := uuid.Parse(bidderStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "missing or malformed X-Guild-ID context header"})
	...
	err = h.auctionService.CancelBid(c.Request.Context(), auctionID, bidderID)
	if err != nil {
		if errors.Is(err, service.ErrAuctionNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
			return
		}
		if errors.Is(err, service.ErrInvalidAuctionID) ||
			errors.Is(err, service.ErrInvalidGuildID) ||
			errors.Is(err, service.ErrRetractLeadingBid) ||
			errors.Is(err, service.ErrActiveBidNotFound) {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		c.Error(err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to cancel bid"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "bid retraction cleared and locked capital released smoothly"})
}
