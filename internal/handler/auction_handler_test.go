...
func TestAuctionHandler_CancelBid(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		},
		{
			...
			headerID: bidderID.String(),
			setupMock: func(svc *mocks.AuctionService) {
				svc.On("CancelBid", mock.Anything, auctionID, bidderID).Return(service.ErrAuctionNotFound).Once()
			},
			wantStatus: http.StatusNotFound,
		},
		{
			name:     "returns bad request for business error",
			pathID:   auctionID.String(),
			headerID: bidderID.String(),
			setupMock: func(svc *mocks.AuctionService) {
				svc.On("CancelBid", mock.Anything, auctionID, bidderID).Return(service.ErrRetractLeadingBid).Once()
			},
			wantStatus: http.StatusBadRequest,
		},
		{
			...
			headerID: bidderID.String(),
			setupMock: func(svc *mocks.AuctionService) {
				svc.On("CancelBid", mock.Anything, auctionID, bidderID).Return(errors.New("db down")).Once()
			},
			wantStatus: http.StatusInternalServerError,
		},
		{
			name:     "cancels bid successfully",
			pathID:   auctionID.String(),
			headerID: bidderID.String(),
			setupMock: func(svc *mocks.AuctionService) {
				svc.On("CancelBid", mock.Anything, auctionID, bidderID).Return(nil).Once()
			},
			wantStatus: http.StatusOK,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.DELETE("/auctions/:id/bid", NewAuctionHandler(svc).CancelBid)

			req := httptest.NewRequest(http.MethodDelete, "/auctions/"+tt.pathID+"/bid", nil)
			req.Header.Set("X-Guild-ID", tt.headerID)
			rec := httptest.NewRecorder()
